...
export REPO_ROOT_OVERRIDE="$ROOT_DIR"
source "${ROOT_DIR}/tools/env_common.sh"
unset REPO_ROOT_OVERRIDE

: "${VK_ICD_FILENAMES:=/usr/share/vulkan/icd.d/lvp_icd.x86_64.json}"
export VK_ICD_FILENAMES

if [[ -z "${GODOT_BIN:-}" ]] || [[ ! -x "${GODOT_BIN:-}" ]] || [[ ! -f "${GODOT_BIN:-}" ]]; then
...
echo "Logging to $LOG_FILE (user:// -> $GODOT_USER_DATA_DIR)"
