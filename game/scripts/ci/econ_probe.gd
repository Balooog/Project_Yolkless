...
func _initialize() -> void:
	var root := get_root()
	var config_node := root.get_node_or_null("Config")
	if config_node == null:
		var config_instance := CONFIG_SCRIPT.new()
		config_instance.name = "Config"
		root.add_child(config_instance)
		config_node = config_instance
	var logger_node := root.get_node_or_null("Logger")
	if logger_node == null:
		var logger_instance := LOGGER_SCRIPT.new()
		logger_instance.name = "Logger"
		root.add_child(logger_instance)
		logger_node = logger_instance
	var env_node := root.get_node_or_null("EnvironmentServiceSingleton")
	if env_node == null:
		var env_instance := ENV_SERVICE_SCRIPT.new()
...
func _configure_seed_and_logging() -> void:
	var config_node = get_root().get_node_or_null("Config")
	if config_node:
	...
	if _logger:
		...
		var seed_value = 0
		if config_node:
			enabled = bool(config_node.logging_enabled)
			force_disable = bool(config_node.logging_force_disable)
			seed_value = int(config_node.seed)
...
func _run_scenario(name: String, seconds: float, strategy: Callable) -> Dictionary:
	...
	economy.dump_triggered.connect(func(amount: float, wallet: float) -> void:
		...
		if _logger:
			_logger.log("INFO", "ECON_PROBE", "shipment", {
				"scenario": name,
				"time": _next_tick_time,
				"amount": amount,
				"wallet": wallet
