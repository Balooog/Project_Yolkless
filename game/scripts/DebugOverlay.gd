...
func _refresh(refresh_tail: bool) -> void:
	...
	var cooldown_left: float = _economy.get_burst_cooldown_left()
	var cooldown_total: float = _economy.get_burst_cooldown_total()
	...
	if logger_node is YolkLogger:
		...
		logging_status = "ON" if stats.get("enabled", false) else "OFF"
