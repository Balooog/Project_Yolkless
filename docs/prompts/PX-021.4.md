...
## Outcome (2025-11-09)
- `SandboxRenderer` now consumes `SandboxEraLibrary` camera + prop data, lerping zoom/offset per era without touching the CA buffers; `Main.gd` feeds tier-driven presets and progress context so transitions follow promotions automatically.
- Era props (barns, silos, towers, etc.) unlock via tier/research thresholds through the new `set_progress_context()` API, and StatsProbe metrics now report the active era plus camera zoom for telemetry.
- EnvPanel comfort tooltips were updated to the requested “Comfort +X.XX % — Era: …” copy, keeping fallback status appended when needed.
- Validation: `python3 tools/docs_lint/check_structure.py`
