...
## Outcome (2025-11-09)
- Added `TopDownRenderer` plus a sandbox view manager so Diorama + Map share the CA buffer; `Main.gd` now exposes a HUD toggle with auto-idle drift (5 min → Map, any input returns to Diorama).
- StatsProbe captures map render timing with `sandbox_render_view_mode=map`, CLI runs can force either view via `--sandbox_view=map`, and EnvPanel shows the contract (“Comfort +X.XX % — Era …”) alongside a map legend.
