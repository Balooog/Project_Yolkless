...
3. Introduce `SandboxService` to listen for factor changes, run the lightweight 128×72 cellular sandbox, and emit `ci_changed(ci, bonus)`.
...
└─► SandboxService
    ...
    ├─ runs CA simulation (128×72 grid)
...
| ------------------- | -------------- | ----------------- | ---------- |
| Early Farm          | `early_farm`   | Morning palette   | baseline |
| Mid Growth          | `mid_growth`   | Brighter daylight | 15s smooth fade |
| Late Serene         | `late_serene`  | Sunset tones      | Soft dusk |
| Wisdom / Space Farm | `colony_alpha` | Off-world light   | New preset |
...
3. Sandbox tick stays ≤2 ms at 128×72.
