# PX-020.4 — Telemetry & Replay Coverage for PX-020 Series
...
## Summary
- Extend the [Telemetry Replay Quickstart](../quickstarts/telemetry_replay_quickstart.md) with PX-020 specific checks (HUD label logging, StatBus fields, automation panel signals) so DocOps can follow a single playbook.
- Describe how `tools/replay_headless.gd` prints the Slot D/F labels every 10 seconds, mirrors those entries into the `hud_labels` summary array, and how CSVs now contain `economy_rate`, `economy_rate_label`, `conveyor_backlog`, `conveyor_backlog_label`.
...
2. Document CSV and JSON schema additions so downstream analytics can ingest PX-020 data (economy rate/backlog, automation target/panel state).
...
| CSV schema note | Detail new columns + units for StatsProbe exports (economy + automation) | Table referencing StatBus catalog |
...
3. Inspect the latest `user://logs/perf/tick_*.csv` and ensure the tail columns include the PX-020 fields (`economy_rate`, `_label`, `conveyor_backlog`, `_label`, `automation_target`, `automation_target_label`, `automation_panel_visible`), plus the newly-added `tier` and `event_id` columns that track the Tier‑1 unlock and PX-016.2 micro-events.
4. Load the generated JSON summary and verify `economy_rate_avg`, `conveyor_backlog_avg`, `automation_target_last`, `automation_panel_visible_ratio`, and `hud_labels` exist with at least one entry (tail entry matches the console `[hud]` log).
