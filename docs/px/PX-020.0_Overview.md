...
| PX-020.3 | Tooltips + copy | Tone-reviewed HUD + panel copy, tooltip clamp map, localization-ready strings | Copy review sign-off, README_UI_BASELINE cross-links, pseudo-loc smoke (HUD screenshots + replay logs) |
...
| PX | Signals / Stats Introduced | Required Validation | Telemetry Expectations |
| --- | --- | --- | --- |
| PX-020.1 | `economy_rate_changed`, `conveyor_backlog_changed`, StatBus `economy_rate`, `conveyor_backlog` | `python3 tools/ui_assert_baseline.py`, `source .env && ./tools/check_only_ci.sh` | `tools/replay_headless.gd` must log Slot D/F labels; StatsProbe CSV includes rate/backlog columns. |
| PX-020.2 | `automation_panel_opened/closed`, `automation_target_changed`, StatBus `automation_target` | `source .env && ./tools/check_only_ci.sh` (Automation sheet focus), manual panel open/close smoke | Replay logs show automation panel visibility + `automation_target` columns; StatBus inspector exposes current string enum. |
| PX-020.3 | Tooltip/copy updates (no new signals) | Copy review + pseudo-loc smoke (`PX-022.1` prep); attach HUD screenshots + copy catalog diff | Replay artifacts include updated localized strings/screenshots for docs. |
| PX-020.4 | Telemetry harness (no new runtime signals) | Replay checklist (see below) attached to PRs | StatsProbe CSV/JSON schema documented; CI bundles latest replay JSON/CSV + `[hud]` console log snippet. |

*Validation command set:* every PX-020 PR must at minimum run `python3 tools/docs_lint/check_structure.py`, `python3 tools/ui_assert_baseline.py`, `source .env && ./tools/check_only_ci.sh`, and the replay checklist defined in [PX-020.4](PX-020.4_Telemetry_Replay.md).  Attach outputs/logs to the PR so DocOps and QA can audit telemetry deltas without repro builds.
...
- PX-020.4 validation hooks (replay/CI expectations) must be in place before declaring any PX-020 branch complete—see the matrix above.
...
- Replay/telemetry expectations (UI baseline + replay checklist + CI command set) are published once here and referenced by every PX-020 sub-doc/Roadmap entry so contributors know the gating steps without hunting across specs.
