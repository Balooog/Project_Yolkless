...
### Tooltip Runtime Update
- PX-020.3 now ships dedicated tooltip tokens for the HUD status trio:
  - `tooltip_power_{normal|warning|critical}`
  - `tooltip_economy_{normal|warning|critical}`
  - `tooltip_backlog_{normal|warning|critical}`
- `game/scripts/Main.gd` instantiates the shared `ui/components/Tooltip.tscn` prefab at runtime (and inside `/scenes/ui_smoke`) so hover text stays consistent with UI copy.  These tooltips render only in gameplay and the UI smoke harness per the PX guardrail—baseline capture scenes remain untouched.
- Tone selection mirrors the same heuristics used for Slot D/F and the power label (`PowerService` warnings, economy rate thresholds, backlog tone from StatBus).  Copy updates flow through `game/data/strings_egg.tsv`, so localization partners can reuse the new keys immediately.
