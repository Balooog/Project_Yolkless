...
- **Status:** Active — core progression loop in place
...
## Recent Progress
- TSV-driven upgrades live under `data/upgrade.tsv` with balance constants loaded by `Balance.gd`.
- `Research.gd` manages RP spend, prerequisites, and multiplier application; prestige points persist via `Save.gd`.
- Economy loop already queries upgrade levels for feed/storage pacing and hot-reloads changes.

## Remaining Work
- Replace proto shop UI with the RM-010 store/research sheets and expose unlock rules to the HUD.
- Author RP gain milestones and nightly telemetry diffing to validate pacing (ties into RM-014 dashboards).
- Document upgrade families / research nodes in `docs/design/Upgrade_Families.md` and add QA coverage once UI ships.
