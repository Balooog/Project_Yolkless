...
- **Status:** Active — baseline auto-burst service running
...
## Recent Progress
- `AutomationService.gd` exposes OFF/MANUAL/AUTO modes, StatBus telemetry, and integrates with `Economy.gd` autoburst cadence.
- Prestige and research multipliers adjust automation cooldowns; save files persist preferences.
- Telemetry captures automation activity via `StatsProbe` to feed RM-014 dashboards.

## Remaining Work
- Build automation management UI within the RM-010 sheets (mode toggles, interval previews, power warnings).
- Sync conveyor delivery callbacks and power throttling once RM-009 and RM-018 refinements land.
- Expand QA coverage for manual overrides and add docs for new StatBus keys in `docs/architecture/StatBus_Catalog.md`.
