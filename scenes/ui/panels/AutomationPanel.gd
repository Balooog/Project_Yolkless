...
var _strings: Node
var _power_limited: bool = false
var _visible_state: bool = false
var _skip_token_styling: bool = false

func _ready() -> void:
	hide()
	_skip_token_styling = OS.has_feature("headless") or OS.has_feature("server") or OS.has_environment("CI")
...
func set_slider_ratio(ratio: float) -> void:
	var percent: float = clamp(ratio, 0.0, 1.0) * 100.0
...
func set_hint_percent(percent: float) -> void:
	var rounded: float = clamp(percent, 0.0, 100.0)
	if _hint_label and _strings:
		var key: String = "automation_panel_slider_hint"
		var fallback_text: String = "Trigger bursts when feed is above {percent}% capacity."
		var hint_seed: String = _hint_label.text if _hint_label.text != "" else fallback_text
		var base: String = String(_strings.call("get_text", key, hint_seed))
		_hint_label.text = base.format({"percent": String.num(rounded, 0)})

func _apply_tokens() -> void:
	if tokens == null:
		tokens = Tokens
	if tokens == null or _skip_token_styling:
		return
...
func _apply_strings() -> void:
	...
	var hint_template: String = String(_strings.call("get_text", "automation_panel_slider_hint", _hint_label.text))
	_hint_label.text = hint_template.format({"percent": String.num(50, 0)})

func _populate_targets() -> void:
	_target_selector.clear()
	var catalog: Node = _strings
	var auto_label: String = "Auto-feed bursts"
	var auto_tooltip: String = "Automatically trigger Feed bursts when production dips below the sweet spot."
	if catalog:
	...
	var manual_label: String = "Manual cadence"
	var manual_tooltip: String = "Stay hands-on."
	if catalog:
...
func _update_state_label() -> void:
	if _strings == null:
		return
	var key: String = "automation_state_enabled" if _toggle.button_pressed else "automation_state_disabled"
	var text: String = String(_strings.call("get_text", key, _state_label.text))
	if _power_limited:
		var suffix: String = String(_strings.call("get_text", "automation_state_power_limited", " — limited by power"))
		text += suffix
...
func _update_slider_value_label(value: float) -> void:
	if _slider_value_label and _strings:
		var template: String = String(_strings.call("get_text", "automation_panel_slider_value", "{percent}%"))
